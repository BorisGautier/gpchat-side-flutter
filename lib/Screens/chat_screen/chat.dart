...
import 'package:gpchat/Screens/contact_screens/SelectContactsToForward.dart';
...
import 'package:gpchat/Screens/chat_screen/utils/deleteChatMedia.dart';
...
import 'package:gpchat/widgets/MultiDocumentPicker/multiDocumentPicker.dart';
import 'package:gpchat/widgets/MultiImagePicker/multiImagePicker.dart';
...
import 'package:gpchat/Screens/chat_screen/utils/audioPlayback.dart';
import 'package:gpchat/Screens/chat_screen/utils/downloadMedia.dart';
import 'package:gpchat/Screens/chat_screen/utils/message.dart';
...
import 'package:gpchat/Services/Providers/seen_state.dart';
...
import 'package:gpchat/Utils/crc.dart';
...
import 'package:gpchat/Screens/chat_screen/Widget/bubble.dart';
...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  String decryptWithCRC(String input) {
    try {
      ...
      GPChat.toast(getTranslated(this.context, 'msgnotload'));
      return '';
    }
    GPChat.toast(getTranslated(this.context, 'msgnotload'));
    return '';
  ...
  readLocal(
    ...
    chatId = GPChat.getChatId(currentUserNo, peerNo);
    ...
    if (content.trim() != '') {
      try {
        ...
        if (encrypted is String) {
          ...
          GPChat.toast('Nothing to encrypt');
        }
  ...
  contextMenuNew(contextForDialog, Map<String, dynamic> mssgDoc, bool isTemp,
    ...
    if ((mssgDoc[Dbkeys.from] == currentUserNo &&
      ...
      tiles.add(ListTile(
          ...
          onTap: () async {
            GPChat.toast(getTranslated(contextForDialog, 'deleting'));
            await FirebaseFirestore.instance
                .collection(DbPaths.collectionmessages)
                .doc(chatId)
                .collection(chatId!)
                ...
                .then((chatDoc) async {
              if (!chatDoc.exists) {
                GPChat.toast('Please reload this screen !');
                  ...
                  if ((mssgDoc.containsKey(Dbkeys.isbroadcast) == true
                    ...
                    await deleteMsgMedia(realtimeDoc, chatId!)
                      ...
                      if (isDeleted == false || isDeleted == null) {
                        GPChat.toast('Could not delete. Please try again!');
                      } else {
                        await FirebaseFirestore.instance
                            .collection(DbPaths.collectionmessages)
                            .doc(chatId)
      ...
      if (list[index].data()!.containsKey(Dbkeys.groupNAME)) {
        ...
        try {
          ...
          if (encrypted is String) {
            ...
            if (content.trim() != '') {
              ...
              if (targetPeer == null) {
                await ChatController.request(
                    ...
                    GPChat.getChatId(widget.currentUserNo,
                        list[tempSendIndex][Dbkeys.phone]));
              }
            ...
            GPChat.toast('Nothing to send');
          }
        } catch (e) {
          setStateIfMounted(() {
            isgeneratingSomethingLoader = false;
          });
          GPChat.toast('Failed to Forward message. Error:$e');
        }
      }
    }
  }
                          ...
                          Container(
                              child: Row(
                                ...
                                Icon(
                                  ...
                                  size: 12,
                                  color: gpchatGrey.withOpacity(0.5),
                                ),
                                SizedBox(
                                  width: 5,
                                ),
                                Text(getTranslated(this.context, 'forwarded'),
                                    maxLines: 1,
                                    style: TextStyle(
                                        color: gpchatGrey.withOpacity(0.7),
          ...
          FlatButton(
              onPressed: () async {
                ...
                if (isUser == null || isUser == false) {
                  GPChat.toast(getTranslated(this.context, 'usernotjoined') +
                      ' $Appname');
                }
              },
  ...
  replyAttachedWidget(BuildContext context, var doc) {
    return Flexible(
      child: Container(
          ...
          decoration: BoxDecoration(
              color: gpchatWhite.withOpacity(0.55),
          ...
          child: Stack(
            children: [
              Container(
                  ...
                  decoration: BoxDecoration(
                      color: gpchatGrey.withOpacity(0.1),
      ...
      child: isLoading
          ? Container(
              ...
              color: DESIGN_TYPE == Themetype.whatsapp
                  ? gpchatBlack.withOpacity(0.0)
                  : gpchatWhite.withOpacity(0.0),
            )
          : Container(),
    );
  }
    ...
    if (chatStatus == ChatStatus.requested.index) {
      return AlertDialog(
        ...
        title: Text(
          ...
          style: TextStyle(color: gpchatBlack),
        ),
        actions: <Widget>[
          // ignore: deprecated_member_use
          FlatButton(
          ...
          FlatButton(
              child: Text(getTranslated(this.context, 'acpt'),
                  style: TextStyle(color: gpchatgreen)),
              onPressed: () {
  ...
  //                     textCapitalization: TextCapitalization.sentences,
  //                     maxLines: null,
  //                     style: TextStyle(fontSize: 18.0, color: gpchatBlack),
  //                     controller: textEditingController,
  //                     decoration: InputDecoration(
  //                       enabledBorder: OutlineInputBorder(
  //                         // width: 0.0 produces a thin "hairline" border
  ...
  List<Widget> sortAndGroupSavedMessages(
    ...
    _groupedSavedMessages.add(Center(
        child: Chip(
            ...
            label: Text(getTranslated(this.context, 'savedconv')))));
                                    ...
                                    children: [
                                      Padding(
                                        padding: const EdgeInsets.fromLTRB(
                                            0, 7, 0, 7),
                                        child: GPChat.avatar(peer, radius: 20),
                                      ),
                                      SizedBox(
                                        width: 7,
                                      ),
                                        ...
                                        onSelected: (dynamic val) {
                                          switch (val) {
                                            ...
                                            case 'remove_wallpaper':
                                              ...
                                              // GPChat.toast('Wallpaper removed.');
                                              break;
                                            case 'set_wallpaper':
                                              Navigator.push(
                                                  context,
