...
class GPChat {
  ...
  static void toast(String message) {
    Fluttertoast.showToast(
        msg: message,
        backgroundColor: gpchatBlack.withOpacity(0.95),
  ...
  static void internetLookUp() async {
    try {
      await InternetAddress.lookup('google.com').catchError((e) {
        GPChat.toast('No internet connection ${e.toString()}');
      });
    } catch (err) {
      GPChat.toast('No internet connection. ${err.toString()}');
    }
  }
      ...
      if (user![Dbkeys.aliasAvatar] == null)
        return (user[Dbkeys.photoUrl] ?? '').isNotEmpty
            ? CircleAvatar(
                ...
                radius: radius)
            : CircleAvatar(
                backgroundColor: gpchatgreen,
                foregroundColor: Colors.white,
                child: Text(getInitials(GPChat.getNickname(user)!)),
                radius: radius,
              );
      return CircleAvatar(
                  ...
                  child: Center(
                      child: Padding(
                          ...
                          child: Text(
                            ...
                            style: TextStyle(color: gpchatWhite, fontSize: 18),
                          ))));
          }
          return child;
        });
